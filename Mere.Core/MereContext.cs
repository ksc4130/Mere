...
namespace Mere.Core
{
    public class MereContext<T> : IDisposable where T : new()
    {
        ...
        public MereContext(MereContextType contextType, MereDataSource dataSource)
        {
            ...
            CurMereTable = MereUtils.CacheCheck<T>();
...
//            Parameters = new List<SqlParameter>();
            ...
            SelectMereColumnsList = CurMereTable.SelectMereColumns;
        }
        ...
        public MereTable<T> CurMereTable { get; set; }
...
//        public string SqlUpdateNotUsingUpdateFields { get { return CurMereTable.SqlUpdateWithoutKey; } }
//        public string SqlUpdate { get { return CurMereTable.SqlUpdateWithoutKey + WhereStr; } }
//
...
//        }
        ...
        public void EnvironmentChanged(string environment)
        {
            ...
            if (CurMereTable == null) CurMereTable = mereTable;

            CurMereDataSource = BaseMereDataSource[environment];
            ServerName = CurMereDataSource.ServerName ?? CurMereTable.ServerName;
            DatabaseName = CurMereDataSource.DatabaseName ?? CurMereTable.DatabaseName;
            TableName = CurMereDataSource.TableName ?? CurMereTable.TableName;

            UserId = CurMereDataSource.UserId ?? CurMereTable.UserId;
            Password = CurMereDataSource.Password ?? CurMereTable.Password;
        }
...
//        }
        ...
        public void SetFields<TFields>()
        {
            ...
            {
                var mereColumn = CurMereTable.GetMereColumn(props[i].Name);
                if (mereColumn == null)
                    continue;
        ...
        public void SetFields<TFields>(Func<T, TFields> newFields)
        {
            ...
            for (var i = 0; i < props.Count; i++)
            {
                var mereColumn = CurMereTable.GetMereColumnByPropertyName(props[i].Name);
                if (mereColumn == null)
                    continue;
        ...
        public void SetFields(dynamic fields)
        {
            ...
            {
                var mereColumn = CurMereTable.GetMereColumn(d.Name);
                if (mereColumn == null)
                    continue;
        ...
        {
            var colName = CurMereTable.GetMereColumn(field).ColumnName;
        ...
        public void AddUpdateField(IEnumerable<KeyValuePair<string, object>> fields, bool filterWithKeyAndOrIdentity)
        {
            ...
            {
                var mereColumn = CurMereTable.GetMereColumnByColumnName(p.Key);
                ...
                if (mereColumn.IsIdentity || mereColumn.IsKey)
                {
                    if (filterWithKeyAndOrIdentity)
                    {
                        ...
                        AddFilter(p.Value, MereOperator.EqualTo);
                    }
        ...
        {
            CurFilterMereColumn = CurMereTable.GetMereColumnByColumnName(colName);
        ...
        {
            CurFilterMereColumn = CurMereTable.GetMereColumn(prop);
            if (CurFilterMereColumn == null)
            {
                ResetCurFilterData();
                return;
        ...
        public void AddFilter(object value, MereOperator mereOperator)
        {
            ...
            var p = Parameters;
            var self = this;
            var toAdd = MereUtils.ParseParamValue(ref self, ref p, CurFilterPropertyName, value, null, SqlOperatorHelper.GetString(mereOperator));
        ...
        public void InitFilterGroupAdd(Expression<Func<T, object>> prop, bool or = false, bool innerGroup = false, int backup = 0)
        {
            ...
            var mereColumn = CurMereTable.GetMereColumn(prop);
            if (mereColumn == null)
            {
                ResetCurFilterData();
                return;
        ...
        public void AddFilterGroup(Expression<Func<T, object>> prop, object value, MereOperator mereOperator)
        {
            BuildingFilterGroup = true;
            CloseFilterGroup = true;
...
//        }
        ...
        public int ExecuteUpdate(T newValues)
        {
            ...
            using (var cmd = GetCommand())
            {
                ...
                foreach (var mereColumn in CurMereTable.SelectMereColumnsNoIdentity())
                {
